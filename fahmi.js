...
const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:Fahmi H\n' + 'ORG:Owner Fbot;\n' + 'TEL;type=CELL;type=VOICE;waid=6283129621297:+62 831-2962-1297\n' + 'END:VCARD';

const vcard1 = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:Fahmi Cog\n' + 'ORG:Co Owner Fbot;\n' + 'TEL;type=CELL;type=VOICE;waid=6285277188863:+62 895-1064-5078\n' + 'END:VCARD';

prefix = '!';
name = 'FBot';
rdaftar = 'THANK YOU FOR REGISTERING TO BECOME A FBot member 😁';
rmenu = 'Oii FBot FRIENDS 👋🏻 DONT FORGET TO DONATE YAA:)';
botinfo = 'FOR INVITE BOTS PLEASE DONATE FIRST YAA:)';
limitt = 999999999;
memberLimit = 2;
ban = [];
premium = ['918281976584@s.whatsapp.net'];
...
async function starts() {
   ...
   fahmi.on('group-participants-update', async (anu) => {
      ...
      try {
         ...
         if (anu.action == 'add') {
            ...
            teks = `*_「 promote deteᴄted 」_*\n@${num.split('@')[0]} Who did the admin add ?, have I joined the group yet!`;
            let buff = await getBuffer(ppimg);
            fahmi.sendMessage(mdata.id, buff, MessageType.image, { caption: teks, contextInfo: { mentionedJid: [num] } });
            ...
            teks = `*_「 demote deteᴄted 」_*\n@${num.split('@')[0]} Who is un admin ?, have I joined the group yet!`;
            let buff = await getBuffer(ppimg);
            fahmi.sendMessage(mdata.id, buff, MessageType.image, { caption: teks, contextInfo: { mentionedJid: [num] } });
            ...
            teks = `*_Goodbye👋_* @${num.split('@')[0]} rest in peace`;
            let buff = await getBuffer(ppimg);
            fahmi.sendMessage(mdata.id, buff, MessageType.image, { caption: teks, contextInfo: { mentionedJid: [num] } });
         }
      } catch (e) {
   ...
   fahmi.on('chat-update', async (mek) => {
      try {
         ...
         mess = {
            wait: '*⏳ Wait is being processed...*',
            success: '*Success...*',
            error: {
               stick: '*Failed, an error occurred while connecting the image to the sticker🥲*',
               Iv: '*Sorry the link is not ᴠalid!*',
            },
            only: {
               group: '*Sorry this command can only be used in groups!*',
               benned: '*Sorry your number was banned, please contact Oᴡner to open your ban*',
               ownerG: '*Sorry this command can only be used by the oᴡner group!*',
               ownerB: '*Sorry this command can only be used by oᴡner bot!* ',
               premium: '*Sorry, this feature is specifically for premium use!*',
               ...
               Badmin: '*Sorry, this command can only be used when the bot is an admin!!*',
               publikG: `*Maaf bot sekarang sudah dipriᴠate oleh oᴡne*\n*untuk lebih jelasnya ketik* \n${prefix}infobot`,
            },
         };

         const botNumber = fahmi.user.jid;
         const ownerNumber = ['918281976584@s.whatsapp.net']; // owner number ubah aja
         ...
         if (messagesLink.includes('bengek')) {
            ...
            fahmi.sendMessage(from, loli, MessageType.audio, { quoted: mek, mi<<metype: 'audio/mp4', ptt: true });
         }
...
͏͏͏͏͏͏͏͏͏͏͏͏͏͏      「 *_Fbot Doᴡnload_* 」
...
Create By _@fahim_`,
               ...
               break;
            case 'zodiak':
               if (args.length < 1) return reply('Enter your zodiac name');
               fahmi.updatePresence(from, Presence.composing);
               if (!isUser) return reply(mess.only.userB);
               teks = body.slice(8);
               try {
            ...
            case 'daftar':
               fahmi.updatePresence(from, Presence.composing);
               if (isUser) return reply('*youve become a friend of Fbot :d*');
               if (isBanned) return reply(mess.only.benned);
               user.push(sender);
               ...
               captionnya = `╭─「 *_registration_* 」\`\`\`\n│ code  sn: \n│TM08GK8PPHBSJDH10J\`\`\`\n│\n│\`\`\`on ${date} ${time}\`\`\`\n│\`\`\`「 name 」: ${pushname2}\`\`\`\n│\`\`\`「 number 」: wa.me/${
                  sender.split('@')[0]
               }\`\`\`\n│\`\`\`to use the bot\`\`\`\n│\`\`\`Please\`\`\`\n│\`\`\`send ${prefix}help/menu\`\`\`\n│\`\`\`\n│total users: ${user.length} persons\`\`\`\n╰────────────────`;
            ...
            case 'owner':
               ...
               fahmi.sendMessage(from, '*_this is the number✌️ :)_*', text, { quoted: mek });
               break;
            case 'coowner':
               fahmi.sendMessage(from, { displayname: 'jeff', vcard: vcard1 }, MessageType.contact, { quoted: mek });
               fahmi.sendMessage(from, '*_Thats my dad eh mean my cooᴡner >-<_*', text, { quoted: mek });
               break;
            case 'fitnah':
               if (isBanned) return reply(mess.only.benned);
               if (!isUser) return reply(mess.only.userB);
            ...
            case 'setname':
               ...
               fahmi.sendMessage(from, `\`\`\`âœ“Successfully Changed Group Name To\`\`\` *${body.slice(9)}*`, text, { quoted: mek });
               break;
            case 'setdesc':
               if (isBanned) return reply(mess.only.benned);
               if (!isUser) return reply(mess.only.userB);
               ...
               fahmi.sendMessage(from, `\`\`\`âœ“Successfully Changed the Group description\`\`\` *${groupMetadata.subject}* Menjadi: *${body.slice(9)}*`, text, { quoted: mek });
               break;
            case 'tts':
               if (isBanned) return reply(mess.only.benned);
               if (!isUser) return reply(mess.only.userB);
               ...
               if (isLimit(sender)) return reply(limitend(pushname2));
               if (args.length < 1) return fahmi.sendMessage(from, 'Which language code is the language, bro? \ N If the language code is not correct, just type * $ {prefix} in language*', text, { quoted: mek });
               const gtts = require('./lib/gtts')(args[0]);
               if (args.length < 2) return fahmi.sendMessage(from, 'Where is the text??', text, { quoted: mek });
               dtt = body.slice(9);
               ranm = getRandom('.mp3');
               rano = getRandom('.ogg');
               dtt.length > 600
            ...
            case 'setpp':
               ...
               reply(mess.wait);
               reply(`\`\`\`âœ“Successfully Changed Group Profile\`\`\` *${groupMetadata.subject}*`);
               break;
            case 'apakah':
               if (isBanned) return reply(mess.only.benned);
               if (!isUser) return reply(mess.only.userB);
            ...
            case 'report':
               ...
               if (pesan.length > 300) return fahmi.sendMessage(from, 'Sorry Text Too Long, Maximum 300 Text', text, { quoted: mek });
               ...
               fahmi.sendMessage('918281976584@s.whatsapp.net', options, text, { quoted: mek });
            ...
            case 'request':
               ...
               if (cfrr.length > 300) return fahmi.sendMessage(from, 'Sorry Text Too Long, Maximum 300 Text', text, { quoted: mek });
               var nomor = mek.participant;
            ...
            case 'nsfwloli':
               try {
                  ...
                  fahmi.sendMessage(from, buffer, image, { quoted: mek, caption: 'Dont make ingredients for comli uncle' });
               } catch (e) {
            ...
            case 'kucing':
               ...
               fahmi.sendMessage(from, buff, image, { quoted: mek, caption: 'meow🐈' });
               await limitAdd(sender);
               break;
            ...
            case 'ytsearch':
               ...
               if (isLimit(sender)) return reply(limitend(pushname2));
               if (args.length < 1) return reply('What are you looking for?  bruh?');
            ...
            case 'addsticker':
               ...
               svst = body.slice(12);
               if (!svst) return reply('Whats the name of the sticker?');
               boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo;
               delb = await fahmi.downloadMediaMessage(boij);
               fs.writeFileSync(`./sticker/${svst}.webp`, delb);
               fahmi.sendMessage(from, `Successfully saved sticker!`, MessageType.text, { quoted: mek });
               break;
            case 'addaudio':
               if (!isOwner) return reply(mess.only.ownerB);
               if (!isQuotedAudio) return reply('Reply the audio');
               gsh = body.slice(7);
               if (!gsh) return reply('whats the name of the file?');
            ...
            case 'add':
               ...
               if (args[0].startsWith('08')) return reply('Please use the country code, bro');
               try {
            ...
            case 'tomp3':
               ...
               fahmi.updatePresence(from, Presence.composing);
               if (!isQuotedVideo) return reply('_*Reply to the video, bro!*_');
               reply(mess.wait);
               encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo;
               media = await fahmi.downloadAndSaveMediaMessage(encmedia);
               ran = getRandom('.mp4');
            ...
            case 'tomp4':
               ...
               exec(`ffmpeg -i ${anum} ${ran}`, (err) => {
                  fs.unlinkSync(anum);
                  if (err) return reply('Failed, when converting the sticker to Video');
                  buffer = fs.readFileSync(ran);
